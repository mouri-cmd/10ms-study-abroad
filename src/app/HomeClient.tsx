...
function InteractiveMapSection({ destinations, lang }: InteractiveMapSectionProps) {
  ...
  const validDestinations = destinations.filter(dest => mapPins[dest.id?.toLowerCase() || dest.slug?.toLowerCase()]);
  const [activeIdx, setActiveIdx] = useState(0);

  const activeDest = validDestinations[activeIdx] || validDestinations[0];
  const activeKey = activeDest ? (activeDest.id?.toLowerCase() || activeDest.slug?.toLowerCase()) : 'usa';
  ...
  const t = {
    bn: {
      ...
      workPermit: "ワーク許可 (Work Permit)", // Defaulting to work permit
    },
    en: {
      mapHeading: "One path, ",
            ...
            <div className="svg-map-wrapper">
              <svg viewBox="0 0 1000 450" className="world-map-svg">
                ...
                {validDestinations.map((dest, idx) => {
                  const key = dest.id?.toLowerCase() || dest.slug?.toLowerCase();
                  const pin = mapPins[key];
                  if (!pin) return null;
              ...
              <div className="country-card-container">
                <div className="country-map-card">
                  <div className="country-card-image-box">
                    ...
                    />
                    {(activeDest.id?.toLowerCase() === 'uk' || activeDest.id?.toLowerCase() === 'usa') && (
...
export default function HomeClient({ destinations }: HomeClientProps) {
  ...
  const homeTestimonials = destinations
    .flatMap((dest) =>
      (dest.testimonials || []).map((test: any) => ({
        ...test,
      ...
      <section className="destinations-section">
        <div className="container">
          ...
          <div className="dest-cards-grid">
            {destinations.slice(0, 4).map((country) => (
              <div key={country.slug || country.id} className="dest-card-upgrad">
                <div className="dest-card-top">
                  <span className="dest-card-flag">{country.flag_emoji}</span>
                  <span className="dest-card-tag-accent bn">{country.id?.toLowerCase() === 'uk' || country.id?.toLowerCase() === 'usa' ? 'Top Choice' : 'Popular'}</span>
                </div>
                <h3 className="dest-card-name">{country.name}</h3>
                <p className="dest-card-highlight-line bn">Tuition: {country.tuitionRange}</p>
                <div className="dest-card-subjects">
                  {country.popular_subjects?.slice(0, 3).map((sub: string, i: number) => (
